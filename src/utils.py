...
def get_top_20_user(data, channel="Random"):
    ...
    plt.title(
        f"Top 20 Message Senders in #{channel} channels", size=15, fontweight="bold"
    )
    ...
    plt.title(
        f"Bottom 10 Message Senders in #{channel} channels", size=15, fontweight="bold"
    )
    plt.xlabel("Sender Name", size=18)
    plt.ylabel("Frequency", size=14)
    plt.xticks(size=12)
...
def draw_avg_reply_count(data, channel="Random"):
    ...
    data.groupby("sender_name")["reply_count"].mean().sort_values(ascending=False)[
        :20
    ].plot(kind="bar", figsize=(15, 7.5))
    plt.title(
        f"Average Number of reply count per Sender in #{channel}",
        size=20,
        fontweight="bold",
    )
    plt.xlabel("Sender Name", size=18)
...
def draw_avg_reply_users_count(data, channel="Random"):
    ...
    data.groupby("sender_name")["reply_users_count"].mean().sort_values(
        ascending=False
    )[:20].plot(kind="bar", figsize=(15, 7.5))
    plt.title(
        f"Average Number of reply user count per Sender in #{channel}",
        size=20,
        fontweight="bold",
    )
    plt.xlabel("Sender Name", size=18)
...
def draw_user_reaction(data, channel="General"):
    data.groupby("sender_name")[["reply_count", "reply_users_count"]].sum().sort_values(
        by="reply_count", ascending=False
    )[:10].plot(kind="bar", figsize=(15, 7.5))
